...
#[pg_guard]
#[no_mangle]
extern "C-unwind" fn mooncake_scan_table_begin(table_id: u32, data: *mut *mut u8, len: *mut usize) {
    let table_id = TableId {
        database_id: unsafe { pgrx::pg_sys::MyDatabaseId.to_u32() },
        table_id,
    };
    let mut bytes = scan_table_begin(table_id).unwrap();
